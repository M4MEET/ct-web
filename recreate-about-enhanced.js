// Recreate the About page with enhanced content
// First delete, then create with new content
...
async function recreateAboutPage() {
  try {
    console.log('Step 1: Deleting existing About page...')
    
    // Delete existing page
    const deleteResponse = await fetch('http://localhost:3002/api/protected/pages', {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        'X-API-Key': 'web-api-key-2024'
      },
      body: JSON.stringify({
        id: 'cmfbkuqkm003yan2u7r9cspd6'
      })
    })

    if (deleteResponse.ok) {
      console.log('✅ Old About page deleted successfully')
    } else {
      console.log('⚠️  Delete failed or page didn\'t exist:', deleteResponse.status)
    }

    console.log('Step 2: Creating enhanced About page...')

    // Create new enhanced page
    const createResponse = await fetch('http://localhost:3002/api/protected/pages', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-API-Key': 'web-api-key-2024'
    ...
    if (createResponse.ok) {
      ...
      console.log('✅ Enhanced About page created successfully!')
      console.log('   Page ID:', result.data?.id)  
      console.log('   Title:', result.data?.title)
      console.log('   Blocks:', result.data?.blocks?.length || 0)
      console.log('   Status:', result.data?.status)
      console.log('')
      console.log('🔗 View at: http://localhost:3002/en/about')
      console.log('🔗 Admin: http://localhost:3001/admin/pages/' + result.data?.id + '/edit')
    } else {
      const error = await createResponse.json()
      console.error('❌ Failed to create enhanced About page:', createResponse.status, error)
    }
    
  } catch (error) {
    console.error('❌ Error recreating About page:', error.message)
  }
}
