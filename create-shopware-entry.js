...
async function createShopwareTechnology() {
  try {
    // First, create the CaseStudy entry
    const shopwareTech = await prisma.caseStudy.create({
      data: {
        slug: 'shopware',
        locale: 'en',
        title: 'Shopware',
    ...
    console.log('✅ Shopware technology entry created:', shopwareTech);
    ...
    console.log('🎉 Shopware technology page created successfully!');
    console.log(`📍 Access it at: /admin/case-studies/${shopwareTech.id}/edit`);
    console.log(`🌐 View on frontend: /en/technologies/shopware`);
    
  } catch (error) {
    console.error('❌ Error creating Shopware technology:', error);
  } finally {
    await prisma.$disconnect();
  }
}

createShopwareTechnology();