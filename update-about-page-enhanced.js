...
// If that doesn't work, let's use a direct database approach
if (!PrismaClient) {
  console.log('Using direct prisma client...')
  const { PrismaClient: DirectPrismaClient } = require('@prisma/client')
...
async function updateAboutPage() {
  try {
    console.log('Updating About page with enhanced content...')

    // First, delete existing About page
    ...
    console.log('✅ Old About page deleted')

    // Create new enhanced About page
    const aboutPage = await prisma.page.create({
      data: {
        id: 'cmfbkuqkm003yan2u7r9cspd6', // Keep same ID for consistency
        slug: 'about',
        locale: 'en',
    ...
    console.log('✅ New About page created:', aboutPage.id)
    ...
    console.log('✅ Enhanced About page with', blocks.length, 'blocks created successfully!')
    ...
    console.log('   Admin edit: http://localhost:3001/admin/pages/' + aboutPage.id + '/edit')
